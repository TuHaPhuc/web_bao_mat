...
# Biến để chuyển đổi giữa chế độ bảo mật và chế độ dễ bị tấn công
# True: Chế độ bảo mật (không có SQL Injection)
# False: Chế độ dễ bị tấn công (có SQL Injection)
SECURE_MODE = True

# Dictionary lưu trữ mật khẩu gốc cho tính năng demo lộ mật khẩu
# LƯU Ý: Đây chỉ là cho mục đích giáo dục, KHÔNG BAO GIỜ lưu trữ mật khẩu dưới dạng plain text trong ứng dụng thực tế
ORIGINAL_PASSWORDS = {
    'admin': 'admin123456',
    'user1': 'password123',
...
def inject_now():
    return {'now': datetime.now(), 'secure_mode': SECURE_MODE}
...
@app.route('/')
def index():
    return render_template('index_switchable.html')
...
def toggle_security():
    ...
    if SECURE_MODE:
        flash('Đã chuyển sang CHẾ ĐỘ BẢO MẬT - SQL Injection đã bị chặn!', 'success')
    else:
        flash('Đã chuyển sang CHẾ ĐỘ DỄ BỊ TẤN CÔNG - SQL Injection có thể được thực hiện!', 'warning')
...
def register():
        ...
        if error is None:
            try:
                ...
                if error is None:
                    ...
                    # Lưu mật khẩu gốc cho demo - CHỈ dùng cho mục đích giáo dục!
    ...
    return render_template('register_switchable.html')

# Đăng nhập - có thể bị tấn công SQL injection hoặc được bảo vệ tùy chế độ
...
def login():
    if request.method == 'POST':
        ...
        if error is None:
            ...
            try:
                ...
                # CHẾ ĐỘ BẢO MẬT: Sử dụng truy vấn tham số hóa để ngăn chặn SQL Injection
                if SECURE_MODE:
                    cursor.execute('SELECT * FROM users WHERE username = ?', (username,))
                ...
                # CHẾ ĐỘ DỄ BỊ TẤN CÔNG: Ghép chuỗi SQL trực tiếp (dễ bị SQL Injection)
                else:
                    # Lỗ hổng SQL Injection: Tạo câu truy vấn bằng cách nối chuỗi
                    ...
                    if user is None:
                        error = 'Tên đăng nhập không tồn tại'
                    # Trong chế độ không bảo mật, cho phép bypass kiểm tra mật khẩu nếu phát hiện SQL injection pattern
    ...
    return render_template('login_switchable.html')
...
def dashboard():
    try:
        ...
        return render_template('dashboard_switchable.html', user=user)
...
def profile():
    try:
        ...
        return render_template('profile_switchable.html', user=user)
...
def change_password():
        ...
        if error is None:
            try:
                ...
                if not check_password_hash(user['password'], current_password):
                    ...
                    # Cập nhật mật khẩu trong dictionary cho demo
                    global ORIGINAL_PASSWORDS
                    ORIGINAL_PASSWORDS[user['username']] = new_password
    ...
    return render_template('change_password_switchable.html')
...
def forgot_password():
    ...
    return render_template('forgot_password_switchable.html')

# Trang quản trị (Admin) - có thể bị tấn công SQL injection hoặc được bảo vệ tùy chế độ
...
def admin_users():
    ...
    # Sử dụng từ điển mật khẩu gốc được lưu ở biến toàn cục
    global ORIGINAL_PASSWORDS
    ...
    try:
        ...
        if SECURE_MODE:
            if not search:
                # Nếu không có tìm kiếm, chỉ hiển thị người dùng hiện tại (theo vai trò của người dùng)
                cursor.execute("SELECT * FROM users WHERE id = ?", (session['user_id'],))
            else:
                # Sử dụng tham số hóa cho tìm kiếm an toàn
                cursor.execute(
            ...
            # Trong chế độ bảo mật, hiển thị thông báo giáo dục
            if search:
                show_debug_info = True
                debug_info = "Tìm kiếm an toàn sử dụng tham số hóa truy vấn - SQL Injection không thể thực hiện"
            ...
            if not search:
                ...
                # Phát hiện tấn công lấy mật khẩu
                    ...
                    try:
                        ...
                        password_leak = []
                        for user in raw_password_leak:
                            # Chuyển đổi từ đối tượng Row sang dict
                            user_dict = dict(user)
        ...
        return render_template('admin_users_switchable.html', users=users, search=search, 
